...
export async function generateMetadata({ params }: { params: { dealType: string; location: string } }): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${categoryPathByDealType(incomingCategorySegment)}/${toCanonicalLocationSegment(district.name)}`,
    },
    openGraph: {
      title,
      description,
      url: `${categoryPathByDealType(incomingCategorySegment)}/${toCanonicalLocationSegment(district.name)}`,
      type: 'website',
    },
  };
}

export default async function DealTypeLocationPage({ params }: { params: { dealType: string; location: string } }) {
  const incomingCategorySegment = params.dealType.trim().toLowerCase();
  if (!isSupportedCategorySegment(incomingCategorySegment)) {
    notFound();
  }
