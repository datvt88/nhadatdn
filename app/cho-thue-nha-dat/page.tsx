...
export default async function RentCategoryPage({
  ...
  const breadcrumbJsonLd = buildBreadcrumbSchema([
    { name: 'NhadatDN', path: '/' },
    { name: 'Cho thuê nhà đất Đà Nẵng', path: '/cho-thue-nha-dat' },
  ]);
  const categoryFacts = buildCategoryFacts(payload.items ?? [], {
    total: Number(payload.total ?? 0),
    dealType: 'cho-thue',
    areaLabel: 'Đà Nẵng',
    categoryLabel: 'danh mục cho thuê nhà đất Đà Nẵng',
  });
  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,_#eef8f8_0%,_#f6fbfb_35%,_#ffffff_100%)]">
      ...
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
        ...
        <CategoryFacts facts={categoryFacts} heading="Câu hỏi thường gặp về cho thuê nhà đất Đà Nẵng" />
      </section>
    </main>
  );
}
